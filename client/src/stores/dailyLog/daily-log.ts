...
// import { Attr, HasMany, Num } from 'pinia-orm/dist/decorators'
...
// import { CompletionEntry } from '../completion/completion-store'
...
//       const relatedDailyLogs = habits[i].completionEntries.filter((y) => y.dailyLogID === this.id)
//       for(let j = 0; j < relatedDailyLogs.length; j++) {
//         relatedDailyLogs[j].status = 1
//         await useRepo(CompletionEntry).piniaStore().axios_updateItem(relatedDailyLogs[j])
//       }
//     }
//   }
